...
class SimpleOrderController extends Controller
{
    ...
    {
        // Log that the controller was instantiated
    ...
    public function submitOrder(Request $request)
    {
        ...
        file_put_contents(storage_path('logs/order-debug.log'), date('Y-m-d H:i:s') . " - submitOrder() method called\n", FILE_APPEND);

        $debugLog = storage_path('logs/order-debug.log');

        try {
            file_put_contents($debugLog, "\n" . date('Y-m-d H:i:s') . " - Order submission started\n", FILE_APPEND);
            ...
            file_put_contents($debugLog, "About to get cart from session...\n", FILE_APPEND);

            // Get cart from session
            $cart = Session::get('cart');

            file_put_contents($debugLog, "Cart retrieved successfully\n", FILE_APPEND);

            if (!$cart) {
                file_put_contents($debugLog, "ERROR: Cart is empty!\n", FILE_APPEND);
                Log::warning('Cart is empty!');
                return redirect()->back()->with('error', 'Your cart is empty');
            }

            file_put_contents($debugLog, "Cart object exists. Total Qty: " . $cart->totalQty . "\n", FILE_APPEND);
